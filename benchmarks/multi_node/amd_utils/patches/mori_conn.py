...
class MoriKVManager(CommonKVManager):
    ...
    def send_state(
        ...
        # ── Bug fix: hybrid-attention `state_item_lens` shape ──────────
        # The framework provides `state_item_lens: List[List[int]]` (one
        # inner list per pool/layer-group). Downstream code in this file
        # (`len(self.kv_args.state_item_lens)` at L~935 and
        # `self.kv_args.state_item_lens[i]` at L~1005, L~1096) expects a
        # FLAT `List[int]` of length `len(state_mem_descs)`. For Qwen3.5
        # MoE-FP8 we observed top=1 outer-list with a 90-int inner list
        # (45 conv-state + 45 SSM-state tensors). Normalize once on entry
        # so the existing index-based code keeps working.
        def _flatten_state_seq(seq):
            out = []
            for entry in seq:
                if entry is None:
                    continue
        ...
        try:
            sil = self.kv_args.state_item_lens
            if sil and isinstance(sil[0], (list, tuple)):
                self.kv_args.state_item_lens = _flatten_state_seq(sil)
            sdpt = getattr(self.kv_args, "state_dim_per_tensor", [])
            if sdpt and isinstance(sdpt[0], (list, tuple)):
                self.kv_args.state_dim_per_tensor = _flatten_state_seq(sdpt)
        except Exception:
            pass  # best-effort; downstream check will surface a clear error

        state_type = getattr(self.kv_args, "state_type", "none")

        # ── Bug fix: legacy `state_type` (singular) returns 'none' ─────
        # The framework migrated `state_type: str` -> `state_types: List[StateType]`
        # (one entry per state component, parallel to state_mem_descs /
        # state_data_ptrs / state_item_lens / state_dim_per_tensor). The
        # Mooncake and NIXL backends already read the new plural list (see
        # `mooncake/conn.py:912` and `nixl/conn.py:1381`); MoRI is the
        # only backend still reading the legacy singular `state_type`,
        # which is no longer populated for many hybrid model paths
        # (Qwen3.5-MoE-FP8 reproducibly leaves it 'none' even though
        # state_mem_descs is non-empty and the scheduler invokes
        # send_state). The per-rank scheduler at line ~1170 only calls
        # send_state when state_indices are present and state_mem_descs
        # is truthy, so reaching this guard means real state to transfer.
        #
        # Safe fix: prefer `kv_args.state_types[0]` (the modern API), fall
        # back to `state_type` (legacy), and surface a clear error if
        # neither is populated. This correctly routes:
        #   * Qwen3.5-MoE  -> StateType.MAMBA -> _send_mamba_state
        #   * GLM-5 (DSA)  -> StateType.DSA   -> _send_swa_dsa_state
        # without guessing.
        state_types_list = getattr(self.kv_args, "state_types", None) or []
        if state_type == "none" and self.state_mem_descs and state_types_list:
            try:
                first = state_types_list[0]
                inferred = getattr(first, "value", None) or str(first).lower()
            except Exception:
                inferred = None
            if inferred in ("mamba", "swa", "dsa"):
                if not getattr(self.__class__, "_logged_state_type_promote", False):
                    try:
                        logger.warning(
                            "[mori-patch] kv_args.state_type='none' but "
                            "state_types=%r and state_mem_descs is non-empty "
                            "(n=%d); using state_types[0]=%r",
                            [getattr(s, "value", str(s)) for s in state_types_list],
                            len(self.state_mem_descs),
                            inferred,
                        )
                    except Exception:
                        pass
                    self.__class__._logged_state_type_promote = True
                state_type = inferred
...
class MoriKVReceiver(CommonKVReceiver):
    ...
    def _register_kv_args(self):
        ...
        # ── Bug fix: hybrid-attention KV-args registration ──────────────
        # The base API declares `state_item_lens: List[List[int]]` (one
        # inner list per layer; sub-list per-tensor item lens) and Mooncake
        # uses pack_int_lists/unpack_int_lists. MoRI was written for the
        # legacy flat `List[int]` and crashes with `struct.error: required
        # argument is not an integer` whenever the framework supplies the
        # nested form (e.g. Qwen3.5 hybrid linear+full attention).
        #
        # Conservative fix: flatten nested to flat-int (one entry per layer)
        # and skip None entries so the existing flat-int wire format and
        # the existing downstream consumers (which index `state_item_lens[i]`
        # as `int`) keep working. We log the observed structure once so we
        # can decide whether a richer per-tensor encoding is needed later.
        _raw_state_item_lens = self.kv_mgr.kv_args.state_item_lens
        _raw_state_dim_per_tensor = getattr(
            self.kv_mgr.kv_args, "state_dim_per_tensor", []
        )
        if not getattr(self.__class__, "_logged_state_shape", False):
            try:
                logger.warning(
                    "[mori-patch] state_item_lens shape: top=%d sample=%r ; "
                    "state_dim_per_tensor shape: top=%d sample=%r",
                    len(_raw_state_item_lens),
                    _raw_state_item_lens[:3],
                    len(_raw_state_dim_per_tensor),
                    _raw_state_dim_per_tensor[:3],
                )
            except Exception:
                pass
            self.__class__._logged_state_shape = True

        def _flatten_skip_none(seq):
            """Flatten List[List[int]] -> List[int]; skip None entries.

            Observed shape on Qwen3.5-MoE-FP8 (logged once above):
                state_item_lens     : [[9216]*45 + [524288]*45]   (outer=1, inner=90)
                state_dim_per_tensor: [[1536]*45 + [8]*45]        (outer=1, inner=90)

            Downstream `send_state` asserts
                len(state_item_lens) == len(state_mem_descs)   # 90 here
            so we must flatten EVERY inner element, not just the first
            (the original assumption of one int per layer is wrong for
            models that pack multiple state tensors per layer).
            """
            out = []
            for entry in seq:
                if entry is None:
        ...
        _flat_state_item_lens = _flatten_skip_none(_raw_state_item_lens)
        _flat_state_dim_per_tensor = _flatten_skip_none(_raw_state_dim_per_tensor)

        packed_state_item_lens = b"".join(
            struct.pack("I", item_len) for item_len in _flat_state_item_lens
        )
        packed_state_dim_per_tensor = b"".join(
            struct.pack("I", dim) for dim in _flat_state_dim_per_tensor
        )
